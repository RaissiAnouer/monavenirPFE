...
export const authAPI = {
  ...
  forgotPassword: async (email: string): Promise<ForgotPasswordResponse> => {
    const response = await client.post<ForgotPasswordResponse>('/api/auth/forgot-password', { email });
    return response.data;
  },

  resetPassword: async (token: string, password: string): Promise<ResetPasswordResponse> => {
    const response = await client.post<ResetPasswordResponse>('/api/auth/reset-password', { token, password });
    return response.data;
  },

  contact: async (formData: ContactForm): Promise<ContactResponse> => {
    try {
      const response = await client.post<ContactResponse>('/api/auth/contact', formData);
      return response.data;
    } catch (error: any) {
