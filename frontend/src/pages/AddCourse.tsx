...
const AddCourse: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!image) {
      newErrors.image = 'Course image is required and must be unique for each course.';
    }
    // Optionally, you could add a check here to compare the image name/content with previous uploads if you store that info
    setErrors(newErrors);
                      ...
                      <div className="space-y-4">
                        ...
                        {image && (
                          <p className="text-xs text-gray-500">
                            Nom généré: {generateUniqueFilename(image).substring(0, 30)}...
                          </p>
                        )}
                      </div>
                    ) : (
