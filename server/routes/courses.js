...
router.post('/', auth, upload.single('image'), async (req, res) => {
  ...
  if (req.file) {
    // Ensure consistent path format and verify file exists
    ...
    if (!fs.existsSync(imagePath)) {
      ...
      return res.status(500).json({ error: 'Image file was not saved correctly' });
    }
    ...
    try {
      ...
      console.error('Image file is not readable:', err);
      return res.status(500).json({ error: 'Image file is not accessible' });
    }
  ...
  try {
    ...
    console.log('Course created successfully with image:', imageUrl);
